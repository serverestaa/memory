...
# Script 3: Forgotten trials comparison
...
print("Loading Chinese SHOW data...")
...
print("Loading German SHOW data...")
...
# Filter for Forgotten trials only (label == 0)
indices_ch_forg = np.nonzero(y_ch == 0)[0]
indices_g_forg = np.nonzero(y_g == 0)[0]

X_ch = X_ch[indices_ch_forg]
X_g = X_g[indices_g_forg]

print(f"Chinese Forgotten - X shape: {X_ch.shape}")
print(f"German Forgotten - X shape: {X_g.shape}")
...
# Process Chinese Forgotten
print("\nProcessing Chinese Forgotten data...")
...
# Process German Forgotten
print("\nProcessing German Forgotten data...")
...
print("\nCalculating Chinese - German Forgotten difference...")
...
vertno_max_rh_diff, time_max_rh_diff = stc_diff.get_peak(hemi='rh')
vertno_max_lh_diff, time_max_lh_diff = stc_diff.get_peak(hemi='lh')

# Use the correct subjects_dir from fsaverage download
subjects_dir = op.dirname(fs_dir)

# Plot Chinese Forgotten
print("\nPlotting Chinese Forgotten brain...")
...
brain_ch.add_foci(vertno_max_lh_ch, coords_as_verts=True, hemi='lh', color='blue',
                  scale_factor=0.6, alpha=0.5)
brain_ch.add_text(0.1, 0.9, f'Chinese SHOW - Forgotten\nwith {method} method', 'title', font_size=10)

input("Press ENTER to continue to German Forgotten...")
try:
    brain_ch.close()
except (AttributeError, RuntimeError):
    pass  # Window already closed

# Plot German Forgotten
print("\nPlotting German Forgotten brain...")
surfer_kwargs = dict(background='white',
    hemi='both', subjects_dir=subjects_dir,
    clim=dict(kind='value', lims=[8, 12, 15]), views='coronal',
...
brain_g.add_foci(vertno_max_lh_g, coords_as_verts=True, hemi='lh', color='blue',
                 scale_factor=0.6, alpha=0.5)
brain_g.add_text(0.1, 0.9, f'German SHOW - Forgotten\nwith {method} method', 'title', font_size=10)
...
# Plot Difference
print("\nPlotting Chinese - German Forgotten difference...")
surfer_kwargs = dict(background='white',
    hemi='both', subjects_dir=subjects_dir,
    clim=dict(kind='value', lims=[8, 12, 15]), views='coronal',
    initial_time=min(time_max_rh_diff, time_max_lh_diff), time_unit='s', size=(800, 800), smoothing_steps=10)

brain_diff = stc_diff.plot(**surfer_kwargs)
brain_diff.add_foci(vertno_max_rh_diff, coords_as_verts=True, hemi='rh', color='red',
                    scale_factor=0.6, alpha=0.5)
brain_diff.add_foci(vertno_max_lh_diff, coords_as_verts=True, hemi='lh', color='blue',
                    scale_factor=0.6, alpha=0.5)
brain_diff.add_text(0.1, 0.9, f'Chinese - German SHOW (Forgotten)\nwith {method} method', 'title', font_size=10)
