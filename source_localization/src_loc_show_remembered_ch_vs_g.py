...
# Script 2: Remembered trials comparison
...
# Filter for Remembered trials only (label == 1)
indices_ch_rem = np.nonzero(y_ch == 1)[0]
indices_g_rem = np.nonzero(y_g == 1)[0]

X_ch = X_ch[indices_ch_rem]
X_g = X_g[indices_g_rem]

print(f"Chinese Remembered - X shape: {X_ch.shape}")
print(f"German Remembered - X shape: {X_g.shape}")
...
# Process Chinese Remembered
print("\nProcessing Chinese Remembered data...")
...
# Process German Remembered
print("\nProcessing German Remembered data...")
...
print("\nCalculating Chinese - German Remembered difference...")
...
# Plot Chinese Remembered
print("\nPlotting Chinese Remembered brain...")
...
brain_ch.add_foci(vertno_max_lh_ch, coords_as_verts=True, hemi='lh', color='blue',
                  scale_factor=0.6, alpha=0.5)
brain_ch.add_text(0.1, 0.9, f'Chinese SHOW - Remembered\nwith {method} method', 'title', font_size=10)

input("Press ENTER to continue to German Remembered...")
try:
    brain_ch.close()
except (AttributeError, RuntimeError):
    pass  # Window already closed

# Plot German Remembered
print("\nPlotting German Remembered brain...")
surfer_kwargs = dict(background='white',
    hemi='both', subjects_dir=subjects_dir,
    clim=dict(kind='value', lims=[8, 12, 15]), views='coronal',
...
brain_g.add_foci(vertno_max_lh_g, coords_as_verts=True, hemi='lh', color='blue',
                 scale_factor=0.6, alpha=0.5)
brain_g.add_text(0.1, 0.9, f'German SHOW - Remembered\nwith {method} method', 'title', font_size=10)
